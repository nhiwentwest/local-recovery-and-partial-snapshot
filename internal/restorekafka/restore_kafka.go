...
func (k *KafkaReader) ReadLatest() (manifest.Manifest, error) {
	r := kafka.NewReader(kafka.ReaderConfig{Brokers: k.brokers, Topic: k.topic, Partition: 0, MinBytes: 1, MaxBytes: 10e6})
	defer r.Close()
...
func ReplayChangelogKafka(st state.Store, brokers []string, topic string, fromOffset int64) restorefs.RestoreResult {
	rd := kafka.NewReader(kafka.ReaderConfig{Brokers: brokers, Topic: topic, Partition: 0, MinBytes: 1, MaxBytes: 10e6})
	defer rd.Close()
	...
	for {
		...
		if err != nil {
			...
			return restorefs.RestoreResult{Applied: applied, Skipped: skipped, Error: fmt.Errorf("read kafka: %w", err)}
		}
		idx++
		if idx <= fromOffset {
			continue
		...
		if err := json.Unmarshal(m.Value, &d); err != nil {
			return restorefs.RestoreResult{Applied: applied, Skipped: skipped, Error: fmt.Errorf("unmarshal delta: %w", err)}
		}
		ok, _, err := st.Apply(d.Key, d.Delta, d.DeltaQty, d.Seq)
		if err != nil {
			return restorefs.RestoreResult{Applied: applied, Skipped: skipped, Error: fmt.Errorf("apply: %w", err)}
		}
		if ok {
			applied++
		} else {
